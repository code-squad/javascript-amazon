class Carousel {
  constructor(input, option, pagination) {
  ...
  setCarouselItem() {
    let originfirstItem = this.items[0],
        originLastItem = this.items[this.itemsInitCount - 1];
  ...
  prevEvent() {
    ...
    let newActiveItem = this.updateActiveItem({
    ...
    if (
      ...
      setTimeout(() => this.moveCloneCarousel(newPointerValue), this.option.speed);
    }
  }
  ...
  attchEventToBtn() {
    this.prevBtn.addEventListener("click", this.prevEvent.bind(this));
    this.nextBtn.addEventListener("click", this.nextEvent.bind(this));
  }
}
